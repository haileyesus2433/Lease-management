...
import { DELETE_LEASE, GET_LEASES, SHARE_LEASE } from "@/constants";

import { convertDate } from "@/lib/utils";
import { useDeleteRQuery } from "@/hooks/useDeleteQuery";
import { usePostMutationRQuery } from "@/hooks/usePostQuery";
import { calculateTotalFromLease } from "@/lib/calculations";
...
const Table = () => {
  ...
  const { mutate: shareApi } = usePostMutationRQuery(
    {} as any,
    `${SHARE_LEASE}/${leaseDetail?.id}`,
    [`lease ${filter} ${page}`]
  );
  ...
  function shareLease(email: string) {
    try {
      shareApi({ email });
    } catch (error) {
      console.log(error);
    ...
    <>
        ...
        <div className="overflow-auto rounded-lg shadow hidden lg:block">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {data?.data?.length > 0 &&
                data?.data?.map((lease: any, index: number) => (
              ...
              >
                ...
                <div className="text-sm text-gray-700">
                  ...
                  <span className="ml-2 text-gray-500">
                    {lease?.annualRentIncreasePercentage}% (Annual rent
                    increase)
                  </span>
                </div>
                <div className="text-sm text-gray-700">
                ...
                <div className="text-sm font-medium text-black">
                  Total:${calculateTotalFromLease(lease)}
                </div>
      ...
      <div className="mt-10">
        <Pagination
          ...
          hasNext={data?.total > limit + data?.data?.length}
        />
      </div>
